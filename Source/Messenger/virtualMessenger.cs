...
class virtualMessenger
{
    ...
    /// </summary>
    private int userID;
    private Dictionary<int, virtualBuddy> Buddies;
    #endregion
    ...
    internal string friendList()
    {
        ...
        var Buddylist = new StringBuilder(Encoding.encodeVL64(200) + Encoding.encodeVL64(200) + Encoding.encodeVL64(600) + "H" + Encoding.encodeVL64(userIDs.Length));
        var Me = new virtualBuddy(userID);
        for (int i = 0; i < userIDs.Length; i++)
        {
            var Buddy = new virtualBuddy(userIDs[i]);
            try
            {
                if (Buddy.Online)
    ...
    internal string friendRequests()
    {
        ...
        var Requests = new StringBuilder(Encoding.encodeVL64(userIDs.Length) + Encoding.encodeVL64(userIDs.Length));
        if(userIDs.Length > 0)
        {
