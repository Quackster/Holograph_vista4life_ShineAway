...
internal class gameLobby
{
    ...
    /// </summary>
    internal Dictionary<int, Game> Games = new Dictionary<int, Game>();
    /// <summary>
    ...
    internal string playerRanks
    {
        get
        {
            ...
            var Ranks = new StringBuilder(Encoding.encodeVL64(Users.Count()));
    ...
    internal string gameList()
    {
        try
        {
            ...
            foreach (var GameItem in Games.Values)
            {
                List.Append(Encoding.encodeVL64(GameItem.ID) + GameItem.Name + Convert.ToChar(2) + Encoding.encodeVL64(GameItem.Owner.roomUID) + GameItem.Owner.User._Username + Convert.ToChar(2));
                if (isBattleBall == false) // SnowStorm game
                    List.Append(Encoding.encodeVL64(GameItem.leftTime));
                List.Append(Encoding.encodeVL64(GameItem.mapID));
                Amounts[(int)GameItem.State]++;
            }
    ...
    internal string getCreateGameSettings()
    {
        if (isBattleBall)
        {
            var Settings = new StringBuilder(Encoding.encodeVL64(4)); // 4 values
