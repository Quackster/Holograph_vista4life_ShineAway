...
public partial class virtualRoom
{
    ...
    internal class FloorItemManager
    {
        ...
        private Dictionary<int, floorItem> _Items = new Dictionary<int, floorItem>();
        /// <summary>
        ...
        internal void removeItem(int itemID, int ownerID)
        {
            if(_Items.ContainsKey(itemID))
            {
                ...
                var Template = catalogueManager.getTemplate(Item.templateID);
        ...
        internal void placeItem(int itemID, int templateID, int X, int Y, byte typeID, byte Z)
        {
            ...
            try
            {
                ...
                {
                    var topItem = _Items[_Room.sqSTACK[X, Y].topItemID()];
                ...
                for (int jX = X; jX < X + Width; jX++)
                {
                    for (int jY = Y; jY < Y + Length; jY++)
                    {
                        ...
                        var jState = _Room.sqSTATE[jX, jY];
                        if (jState != squareState.Open)
                        {
                            if (jState == squareState.Blocked)
                            {
                                ...
                                {
                                    var stackTopItem = _Items[_Room.sqSTACK[jX, jY].topItemID()];
                                    var topItemTemplate = catalogueManager.getTemplate(stackTopItem.templateID);
                                    if (topItemTemplate.topH == 0 || topItemTemplate.typeID == 2 || topItemTemplate.typeID == 3) // No stacking on seat/bed
                                        return;
                                    else
                                    {
                                        if (stackTopItem.H + topItemTemplate.topH > H) // Higher than previous topheight
                                            H = stackTopItem.H + topItemTemplate.topH;
                                    }
                                }
                            }
                            else if (jState == squareState.Rug && _Room.sqSTACK[jX, jY] != null)
                            {
                                double jH = _Items[_Room.sqSTACK[jX, jY].topItemID()].H + 0.1;
                                if (jH > H)
                                    H = jH;
                            }
                            else // Seat etc
        ...
        internal void relocateItem(int itemID, int X, int Y, byte Z)
        {
            try
            {
                ...
                if (_Room.sqSTACK[X, Y] != null)
                {
                    ...
                    if (topItem != Item)
                    {
                        var topTemplate = catalogueManager.getTemplate(topItem.templateID);
                ...
                for (int jX = X; jX < X + Width; jX++)
                {
                    for (int jY = Y; jY < Y + Length; jY++)
                    {
                        ...
                        if (jState != squareState.Open)
                        {
                            ...
                            {
                                var stackTopItem = _Items[Stack.topItemID()];
                                if (stackTopItem != Item)
                                {
                                    var topItemTemplate = catalogueManager.getTemplate(stackTopItem.templateID);
                                    if (topItemTemplate.typeID == 1 && topItemTemplate.topH > 0)
                                    {
                                        if (stackTopItem.H + topItemTemplate.topH > H)
                                            H = stackTopItem.H + topItemTemplate.topH;
                                    }
                                    else
                                    {
                ...
                for (int jX = Item.X; jX < Item.X + oldWidth; jX++)
                {
                    for (int jY = Item.Y; jY < Item.Y + oldLength; jY++)
                    {
                        ...
                        if (Stack != null && Stack.Count > 1)
                        {
                            if (itemID == Stack.bottomItemID())
                            {
                                if(catalogueManager.getTemplate(_Items[Stack.topItemID()].templateID).typeID == 2)
                                    _Room.sqSTATE[jX,jY] = squareState.Seat;
                                else
                            ...
                            {
                                var belowItem = _Items[Stack.getBelowItemID(itemID)];
                                byte belowTypeID = catalogueManager.getTemplate(belowItem.templateID).typeID;

                                _Room.sqSTATE[jX, jY] = (squareState)belowTypeID;
                                if (belowTypeID == 2 || belowTypeID == 3)
                                {
                                    _Room.sqITEMROT[jX, jY] = belowItem.Z;
                                    _Room.sqITEMHEIGHT[jX, jY] = belowItem.H + catalogueManager.getTemplate(belowItem.templateID).topH;
                                }
                                else if (belowTypeID == 4)
                                    _Room.sqITEMHEIGHT[jX, jY] = belowItem.H;
                            }
                            Stack.Remove(itemID);
                            _Room.sqSTACK[jX, jY] = Stack;
        ...
        internal string Items
        {
            get
            {
                var itemList = new StringBuilder(Encoding.encodeVL64(_Items.Count));
