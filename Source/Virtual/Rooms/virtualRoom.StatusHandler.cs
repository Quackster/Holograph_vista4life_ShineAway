        ...
        try
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                #region Virtual user status handling
                {
                    ...
                    var stateMap = (squareState[,])sqSTATE.Clone();
                    try
                    {
                    ...
                    var nextCoords = new Pathfinding.Pathfinder(stateMap, sqFLOORHEIGHT, sqUNIT).getNext(roomUser.X, roomUser.Y, roomUser.goalX, roomUser.goalY);
                    ...
                    if (nextCoords == null) // No next steps found, destination reached/stuck
                    {
                        ...
                        {
                            var Trigger = sqTRIGGER[roomUser.X, roomUser.Y];
                    ...
                    else // Next steps found by pathfinder
                    {
                        ...
                        var nextState = sqSTATE[nextX, nextY];
                ...
                foreach (var roomBot in _Bots.Values)
                {
                    ...
                    var nextCoords = new Pathfinding.Pathfinder(stateMap, sqFLOORHEIGHT, sqUNIT).getNext(roomBot.X, roomBot.Y, roomBot.goalX, roomBot.goalY);
