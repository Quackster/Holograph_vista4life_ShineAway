                ...
                case "Bv": // Enter room - loading screen advertisement
                    {
                        ...
                        if (Config.Rooms_LoadAvertisement_img == "")
                            sendData(HabboPackets.ROOM_ADVERTISEMENT + "0");
                        else
                            sendData(HabboPackets.ROOM_ADVERTISEMENT + Config.Rooms_LoadAvertisement_img + HabboProtocol.TAB_SEPARATOR + Config.Rooms_LoadAvertisement_uri);
                    }
                    break;
                ...
                case "@B": // Enter room - determine room and check state + max visitors override
                    {
                        ...
                        else
                        {
                            ...
                            if (_teleporterID == 0)
                            {
                                ...
                                {
                                    sendData(HabboPackets.ROOM_FULL + "Kc");
                                    return true;
                                }
                                ...
                                if (nowVisitors > 0)
                                {
                                    ...
                                    if (nowVisitors >= maxVisitors && rankManager.containsRight(_Rank, "fuse_enter_full_rooms") == false)
                                    {
                                        if (isPublicroom == false)
                                            sendData(HabboPackets.ROOM_FULL + HabboProtocol.BOOL_TRUE);
                                        else
                        ...
                        if (_inPublicroom == false)
                        {
                            ...
                            if (_teleporterID == 0 && _isOwner == false && rankManager.containsRight(_Rank, "fuse_enter_locked_rooms") == false)
                            {
                                ...
                                if (DB.checkExists("SELECT roomid FROM room_bans WHERE roomid = '" + _roomID + "' AND userid = '" + userID + "'"))
                                {
                                    ...
                                    {
                                        sendData(HabboPackets.ROOM_FULL + "PA");
                                ...
                                else if (accessFlag == 2) // Password
                                {
                                    ...
                                    if (givenPassword != roomPassword) { sendData(HabboPackets.ROOM_PASSWORD_REQUIRED + "Incorrect flat password"); return true; }
                                }
                            }
                            _ROOMACCESS_SECONDARY_OK = true;
                ...
                case "Ab": // Answer guestroom doorbell
                    {
                        ...
                        if (letIn)
                        {
                            ...
                            Room.sendDataToRights(HabboPackets.ROOM_CLOSED + ringer + HabboProtocol.FIELD_SEPARATOR);
                ...
                case "@{": // Enter room - guestroom - guestroom only data: model, wallpaper, floor, landscape, rights, room votes // Updated by Su-la-ke
                    {
                        if (_ROOMACCESS_SECONDARY_OK && _inPublicroom == false)
                        {
                            ...
                            sendData(HabboPackets.USER_STATUS + "landscape/" + Landscape);
                            if (Wallpaper > 0)
                                sendData(HabboPackets.USER_STATUS + "wallpaper/" + Wallpaper);
                            if (Floor > 0)
                                sendData(HabboPackets.USER_STATUS + "floor/" + Floor);
                            ...
                            sendData(HabboPackets.VOTE_UPDATE + Encoding.encodeVL64(voteAmount));
                            sendData(HabboPackets.EVENT_INFO + eventManager.getEvent(_roomID));
                        }
                        break;
                    }
                ...
                case "A~": // Enter room - get room advertisement
                    {
                        if (_inPublicroom && DB.checkExists("SELECT roomid FROM room_ads WHERE roomid = '" + _roomID + "'"))
                        {
                            ...
                            sendData(HabboPackets.ROOM_INTERSTITIAL + advImg + HabboProtocol.TAB_SEPARATOR + advUri);
                        }
                        else
                            sendData(HabboPackets.ROOM_INTERSTITIAL + "0");
                        break;
                    }
                ...
                case "@|": // Enter room - get roomclass + get heightmap
                    {
                        if (_ROOMACCESS_SECONDARY_OK)
                        {
                            ...
                            sendData(HabboPackets.FLAT_CATEGORY + Room.Heightmap);
                        ...
                        else
                        {
                            ...
                            {
                                sendData(HabboPackets.FLAT_CATEGORY + gamePlayer.Game.Heightmap);
                ...
                case "@}": // Enter room - get items
                    {
                        ...
                        {
                            sendData(HabboPackets.ROOM_FLOOR_ITEMS + Room.PublicroomItems);
                            sendData(HabboPackets.ROOM_WALL_ITEMS + Room.Flooritems);
                        }
                        break;
                    }
                    ...
                    {
                        {
                            if (_ROOMACCESS_SECONDARY_OK && Room != null)
                            {
                                sendData(HabboPackets.FRIEND_UPDATE + Room.Groups);
                                if (Room.Lobby != null)
                                {
                                ...
                                sendData(HabboPackets.ROOM_INFO + HabboProtocol.BOOL_FALSE);
