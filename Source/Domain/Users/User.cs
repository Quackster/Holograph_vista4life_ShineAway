...
using Holo.Domain.Users.Items;
using Holo.Domain.Users.Messenger;
using Holo.Domain.Games;
...
namespace HolographEmulator.Domain.Users
{
    ...
    public class User
    {
        ...
        internal Room Room;
        /// <summary>
        /// The RoomUser that represents this virtual user in room. Contains in-room only objects such as position, rotation and walk related objects.
        /// </summary>
        ...
        private static void HandleClientVersion(User user, PacketReader reader)
        {
            user.sendData("DUIH");
        }

        private static void HandleClientInfo(User user, PacketReader reader)
        {
        ...
        private static void HandleSSORequest(User user, PacketReader reader)
        {
            user.sendData(PacketBuilder.Create("DA")
        ...
        private static void HandleLogin(User user, PacketReader reader)
        {
        ...
        // Logged in handlers
        private static void HandleRequestDate(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            user.sendData("Bc" + DateTime.Today.ToShortDateString());
        }

        private static void HandleInitializeMessenger(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleInitializeClub(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            user.refreshClub();
        }

        private static void HandleRefreshAppearance(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            user.refreshAppearance(false, true, false);
        }

        private static void HandleRefreshValueables(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            user.refreshValueables(true, true);
        }

        private static void HandleRefreshBadges(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            user.refreshBadges();
        }

        private static void HandleRefreshGroupStatus(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            user.refreshGroupStatus();
        }

        private static void HandleRecyclerSetup(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            user.sendData("Do" + RecyclerManager.setupString);
        }

        private static void HandleRecyclerSession(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            user.sendData("Dp" + RecyclerManager.sessionString(user.userID));
        }

        private static void HandleSetGuideAvailable(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            _userDataAccess.SetGuideAvailability(user.userID, true);
        }

        private static void HandleSetGuideUnavailable(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            _userDataAccess.SetGuideAvailability(user.userID, false);
        }

        private static void HandleBuyGameTickets(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            ...
            {
                User receiverUser = UserManager.getUser(receiverID);
        ...
        private static void HandleRedeemVoucher(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleSearchConsole(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            user.sendData("HR" + "L");
        }

        private static void HandleSearchInConsole(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleRequestFriend(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Messenger == null) return;
            ...
            if (toID > 0 && !user.Messenger.hasFriendRequests(toID) && !user.Messenger.hasFriendship(toID))
            {
                ...
                User targetUser = UserManager.getUser(toID);
                if (targetUser != null)
                {
        ...
        private static void HandleAcceptFriendRequest(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Messenger == null) return;
        ...
        private static void HandleDeclineFriendRequest(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Messenger == null) return;
        ...
        private static void HandleRemoveBuddy(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Messenger == null) return;
            ...
            {
                User buddy = UserManager.getUser(buddyID);
        ...
        private static void HandleSendInstantMessage(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Messenger == null) return;
        ...
        private static void HandleRefreshFriendList(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Messenger == null) return;
            user.sendData("@M" + user.Messenger.getUpdates());
        }
        private static void HandleFollowBuddy(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Messenger == null) return;
            ...
            if (user.Messenger.hasFriendship(id))
            {
                ...
                {
                    User targetUser = UserManager.getUser(id);
        ...
        private static void HandleInviteBuddies(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            ...
            {
                User buddy = UserManager.getUser(id);
        ...
        private static void HandleNavigateRooms(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleRequestCategoryIndex(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleRefreshRecommendedRooms(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleViewOwnRooms(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleSearchRooms(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleGetRoomDetails(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleInitializeFavorites(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleAddFavorite(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleRemoveFavorite(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleGetEventSetup(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            user.sendData(PacketBuilder.Create("Ep").AppendVL64(EventManager.categoryAmount));
        }

        private static void HandleToggleEventButton(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleCheckEventCategory(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleOpenEventCategory(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleCreateEvent(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleEndEvent(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleCreateRoomPhase1(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleCreateRoomPhase2(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleModifyRoom(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleTriggerRoomModify(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleEditRoomCategory(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleDeleteRoom(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleWhoIsInRoom(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleLeaveRoom(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleRoomAdvertisement(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleEnterRoom(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleEnterRoomTeleporter(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            user.sendData("@S");
        }
        private static void HandleCheckRoomAccess(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleAnswerDoorbell(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            ...
            User ringerData = UserManager.getUser(ringer);
        ...
        private static void HandleEnterRoomData(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleGetRoomAd(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleGetRoomClass(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleGetRoomItems(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleGetRoomGroupBadges(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (!user._ROOMACCESS_SECONDARY_OK || user.Room == null)
                return;
        ...
        private static void HandleGetWallItems(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (!user._ROOMACCESS_SECONDARY_OK || user.Room == null)
                return;
        ...
        private static void HandleAddUserToRoom(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleModTool(User user, PacketReader reader) { if (!user._isLoggedIn) return; /* TODO: Implement */ }
        private static void HandleSendCFHMessage(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleDeleteCFHMessage(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleSubmitCFHMessage(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleReplyCFH(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            ...
            User toVirtualUser = UserManager.getUser(toUserID);
        ...
        private static void HandleDeleteCFH(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (!RankManager.containsRight(user._Rank, "fuse_receive_calls_for_help"))
                return;
        ...
        private static void HandlePickupCFH(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleGoToCFHRoom(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (!RankManager.containsRight(user._Rank, "fuse_receive_calls_for_help"))
                return;
        ...
        private static void HandleIgnoreUser(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            try
            {
                reader.Skip(2); // Skip characters after header
                string username = DB.Stripslash(reader.PopString());
                User target = UserManager.getUser(username);
        ...
        private static void HandleUnignoreUser(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            try
            {
        ...
        private static void HandleRotateUser(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleWalkTo(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleClickDoor(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Room == null || user.roomUser == null) return;
        ...
        private static void HandleSelectSwimmingOutfit(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            ...
            if (trigger.Object == "curtains1" || trigger.Object == "curtains2")
            {
                ...
                User userObj = UserManager.getUser(user.userID);
                if (userObj != null)
                    userObj.refreshAppearance(true, true, true);
            }
        }

        private static void HandleSwitchBadge(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Room == null || user.roomUser == null) return;
        ...
        private static void HandleGetTags(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleGetGroupBadgeDetails(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Room == null || user.roomUser == null) return;
        ...
        private static void HandleIgnoreUser2(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            try
            {
        ...
        private static void HandleUnignoreUser2(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            try
            {
        ...
        private static void HandleStopStatus(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleWave(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Room == null || user.roomUser == null) return;
        ...
        private static void HandleDance(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Room == null || user.roomUser == null) return;
            if (user.statusManager == null) return;
        ...
        private static void HandleCarryItem(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleAnswerPoll(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Room == null || user.roomUser == null) return;
        ...
        private static void HandleSay(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleShout(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleWhisper(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleShowSpeechBubble(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Room == null || user.roomUser == null) return;
            if (!user.roomUser.isTyping)
        ...
        private static void HandleHideSpeechBubble(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Room == null || user.roomUser == null) return;
            if (user.roomUser.isTyping)
        ...
        private static void HandleGiveRights(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleTakeRights(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Room == null || user.roomUser == null || user._inPublicroom || !user._isOwner)
                return;
        ...
        private static void HandleKickUser(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleKickAndBan(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleVoteRoom(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleOpenCatalogue(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            user.sendData("A~" + CatalogueManager.getPageIndex(user._Rank));
        }

        private static void HandleOpenCataloguePage(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandlePurchaseItem(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleRecyclerProceed(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleRecyclerRedeem(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (!Config.enableRecycler || (user.Room != null && !RecyclerManager.sessionExists(user.userID)))
                return;
        ...
        private static void HandleHand(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Room == null || user.roomUser == null) return;
        ...
        private static void HandleHand2(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Room == null || user.roomUser == null) return;
        ...
        private static void HandleApplyWallpaper(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (!user._hasRights || user._inPublicroom || user.Room == null || user.roomUser == null)
                return;
        ...
        private static void HandlePlaceItem(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (!user._hasRights || user._inPublicroom || user.Room == null || user.roomUser == null)
                return;
        ...
        private static void HandlePickupItem(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleMoveItem(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (!user._hasRights || user._inPublicroom || user.Room == null || user.roomUser == null)
                return;
        ...
        private static void HandleToggleWallItem(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user._inPublicroom || user.Room == null || user.roomUser == null)
                return;
        ...
        private static void HandleToggleFloorItem(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            try
            {
        ...
        private static void HandleOpenPresent(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (!user._isOwner || user._inPublicroom || user.Room == null || user.roomUser == null)
                return;
        ...
        private static void HandleRedeemCreditItem(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (!user._isOwner || user._inPublicroom || user.Room == null || user.roomUser == null)
                return;
        ...
        private static void HandleEnterTeleporter(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user._inPublicroom || user.Room == null || user.roomUser == null)
                return;
        ...
        private static void HandleCloseDice(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleSpinDice(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Room == null || user.roomUser == null || user._inPublicroom)
                return;
        ...
        private static void HandleSpinWheel(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleActivateLoveShuffler(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Room == null || user.roomUser == null || user._inPublicroom)
                return;
        ...
        private static void HandleOpenStickie(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Room == null || user.roomUser == null || user._inPublicroom)
                return;
        ...
        private static void HandleEditStickie(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (!user._hasRights || user.Room == null || user.roomUser == null || user._inPublicroom)
                return;
        ...
        private static void HandleDeleteStickie(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleLidoVoting(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Room == null || user.roomUser == null) return;
            if (user.statusManager == null) return;
        ...
        private static void HandleInitializeSoundMachine(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleEnterRoomPlaylist(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleGetSongData(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Room == null || user.Room.floorItemManager.soundMachineID <= 0)
                return;
        ...
        private static void HandleSavePlaylist(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (!user._isOwner || user.Room == null || user.Room.floorItemManager.soundMachineID <= 0)
                return;
        ...
        private static void HandleBurnSong(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (!user._isOwner || user.Room == null || user.Room.floorItemManager.soundMachineID <= 0)
                return;
        ...
        private static void HandleDeleteSong(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (!user._isOwner || user.Room == null || user.Room.floorItemManager.soundMachineID <= 0)
                return;
        ...
        private static void HandleInitializeSongEditor(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (!user._isOwner || user.Room == null || user.Room.floorItemManager.soundMachineID <= 0)
                return;
            
            user.songEditor = new SongEditor(user.Room.floorItemManager.soundMachineID, user.userID);
        ...
        private static void HandleAddSoundSet(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleSaveNewSong(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.songEditor == null || !user._isOwner || user.Room == null || user.Room.floorItemManager.soundMachineID <= 0)
                return;
        ...
        private static void HandleRequestEditSong(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (!user._isOwner || user.Room == null || user.Room.floorItemManager.soundMachineID <= 0)
                return;
        ...
        private static void HandleSaveEditedSong(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.songEditor == null || !user._isOwner || user.Room == null || user.Room.floorItemManager.soundMachineID <= 0)
                return;
        ...
        private static void HandleStartTrading(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleOfferTradeItem(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Room == null || user.roomUser == null || user._tradePartnerRoomUID == -1)
                return;
        ...
        private static void HandleDeclineTrade(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Room == null || user.roomUser == null || user._tradePartnerRoomUID == -1)
                return;
        ...
        private static void HandleAcceptTrade(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Room == null || user.roomUser == null || user._tradePartnerRoomUID == -1)
                return;
        ...
        private static void HandleAbortTrade(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Room == null || user.roomUser == null || user._tradePartnerRoomUID == -1)
                return;
        ...
        private static void HandleRefreshGameList(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleCheckoutGame(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleRequestNewGame(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Room == null || user.roomUser == null || user.Room.Lobby == null || user.GamePlayer != null)
                return;
        ...
        private static void HandleProcessNewGame(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Room == null || user.roomUser == null || user.Room.Lobby == null || user.GamePlayer != null)
                return;
        ...
        private static void HandleSwitchTeam(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleLeaveGame(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            user.leaveGame();
        }

        private static void HandleKickPlayer(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Room == null || user.Room.Lobby == null || user.GamePlayer == null || 
        ...
        private static void HandleStartGame(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (user.Room == null || user.Room.Lobby == null || user.GamePlayer == null || 
        ...
        private static void HandleGameMove(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleGameRestart(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
        ...
        private static void HandleToggleMoodlight(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (!user._isOwner)
                return;
        ...
        private static void HandleLoadMoodlight(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
            if (!user._isOwner)
                return;
        ...
        private static void HandleUpdateDimmer(User user, PacketReader reader)
        {
            if (!user._isLoggedIn) return;
